...
encode_message = encode(input_str, prob_range)

decoded_message = decode(encode_message,prob_range,len(input_str))

with open("encoded.txt", "w") as file_in:
    file_in.write(str(encode_message))

with open("decoded.txt", "w") as file_in:
    file_in.write(decoded_message)